...
Usage:
    python copy_rds_cluster_snapshot.py <source_snapshot_name> [--target-snapshot-name TARGET_SNAPSHOT_NAME]
                                        --shared-kms-key SHARED_KMS_KEY
                                        --source-account-id SOURCE_ACCOUNT_ID --target-account-id TARGET_ACCOUNT_ID
                                        [--target-kms-key TARGET_KMS_KEY] [--source-profile SOURCE_PROFILE]
                                        [--target-profile TARGET_PROFILE] [--source-region SOURCE_REGION]
...
Options:
    --target-snapshot-name TARGET_SNAPSHOT_NAME The name of the target RDS cluster snapshot (optional).
    ...
    --source-region SOURCE_REGION               The AWS region of the source RDS cluster snapshot (default: us-east-1).
    --target-region TARGET_REGION               The AWS region of the target RDS cluster snapshot (optional).
...
app = typer.Typer(
    help="Copy an RDS cluster snapshot between AWS accounts."
)
...
def main(
    source_snapshot_name: str = typer.Argument(
        ..., help="The name of the source RDS cluster snapshot."
    ),
    target_snapshot_name: Optional[str] = typer.Option(
        None,
        "--target-snapshot-name",
        help="The name of the target RDS cluster snapshot (optional).",
    ),
    ...
    source_region: str = typer.Option(
        "us-east-1",
        "--source-region",
        help="The AWS region of the source RDS cluster snapshot.",
    ),
    target_region: Optional[str] = typer.Option(
        None,
        "--target-region",
        help="The AWS region of the target RDS cluster snapshot (optional).",
    ),
):
    """
    Copy an RDS cluster snapshot between AWS accounts.
    """
    try:
        ...
        target_snapshot_arn = copy_snapshot(
            target_rds_client,
            f"arn:aws:rds:{source_region}:{source_account_id}:cluster-snapshot:{shared_snapshot_name}",
